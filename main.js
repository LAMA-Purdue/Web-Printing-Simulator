const PrintingSimulator = (() => {
  ...
  function getNextPoint() {
    ...
    try {
      x = 2.75 * (parseInt(fileByLine[lineCounter].split("X")[1].split(" ")[0]));
    } catch(e) {
      x = prevVector[0];
    }
    try {
      y = 2.75 * (parseInt(fileByLine[lineCounter].split("Y")[1].split(" ")[0]));
    } catch(e) {
      y = prevVector[1];
    }
...
/* Expose the functions that p5.js calls, and call associated simulator functions */
function setup() {PrintingSimulator.setupScene();}
function draw() {PrintingSimulator.drawScene();}
function keyPressed() {PrintingSimulator.keyWasPressed();}
function keyReleased() {PrintingSimulator.keyWasReleased();}